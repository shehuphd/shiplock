...
Last updated: 2026-09-01 12:46:24 UTC
...
| `.github/workflows/gate.yml` | Reusable release gate (`workflow_call`): `check` job runs the deterministic checks; `audit` job runs the semantic audit through a declared agent CLI (claude or codex, neutral key secrets), continues an interrupted run on an optional fallback runner via the audit's progress log, opens an issue on failure, and reports per-attempt token usage in the job summary. |
...
| `.github/workflows/release-gate.yml` | Shiplock consuming its own `gate.yml` (consumer zero) on push, PR, and manual dispatch; pushes audit on the cheap model, dispatches on the stronger one. |
...
| `shiplock.toml` | Shiplock's own gate config: doc surfaces, sweep globs, version package, architecture doc, manifest sources, coverage entries. |
